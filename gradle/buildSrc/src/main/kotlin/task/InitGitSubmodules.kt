...
internal fun Project.createInitGitSubmodulesTask(
    ...
    doLast {
        var exit = gitCmd("submodule", "update", "--init", "--force", printOut = true).exitCode
        exit += gitCmd("submodule", "update", "--init", "--recursive", "--force", dir = upstreamDir, printOut = true).exitCode
        if (exit != 0) {
